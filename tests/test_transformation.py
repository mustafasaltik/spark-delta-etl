...
class TestTransformation(unittest.TestCase):
    ...
    def test_drop_null_values(self):
        ...
        self.assertFalse(
            result_df.filter(result_df.transaction_date.isNull()).count() > 0
        )
    ...
    def test_convert_to_timestamp(self):
        ...
        result_df = self.transformation.convert_to_timestamp(
            self.sample_df, "transaction_date", "yyyy-MM-dd"
        )
        ...
        converted_dates = (
            result_df.filter(result_df.transaction_date.isNotNull())
