...
def main():
    ...
    logging.info("Accounts data is going to be cleaned..")
    accounts_df_cleaned = transformation.drop_null_values(accounts_df)

    logging.info("Transactions data is going to be cleaned..")
    transactions_df_cleaned = transformation.drop_null_values(transactions_df)

    logging.info("Customers data is going to be cleaned..")
    customers_df_cleaned = transformation.drop_null_values(customers_df)

    # Date conversion
    transactions_df_cleaned = transformation.convert_to_timestamp(
        transactions_df_cleaned, "transaction_date", "yyyy-MM-dd"
    ).withColumn("year_month", date_format(col("transaction_date"), "yyyy-MM"))
    customers_df_cleaned = transformation.convert_to_timestamp(
        customers_df_cleaned, "join_date", "yyyy-MM-dd"
    )

    # Merge data
    merged_data = transactions_df_cleaned.join(
    ...
    merged_data.groupBy("account_id", "year_month", "transaction_type").agg(
        sum("amount").alias("total_amount")
    ).write.format("delta").mode("overwrite").save(
        config["output"]["aggregated_data"]
        + "/total_amounts_per_acc_yearmonth_trx_type"
    )

    merged_data.groupBy("account_type", "year_month").agg(
        sum("balance").alias("total_balance")
    ).write.format("delta").mode("overwrite").save(
        config["output"]["aggregated_data"] + "/total_balance_per_acc_type"
    )

    merged_data.groupBy("customer_id").agg(
        avg("amount").alias("avg_transaction_amount")
    ).write.format("delta").mode("overwrite").save(
        config["output"]["aggregated_data"] + "/avg_transaction_amt_per_customer"
    )

    # Data enrichment and export process
    trx_classification = merged_data.withColumn(
        "transaction_classification",
        when(col("amount") > 1000, "high")
        .when(
            (col("amount") >= 500) & (col("amount") <= 1000),
            "medium",
        )
        .otherwise("low"),
    )

    trx_classification.write.format("delta").mode("overwrite").save(
        config["output"]["enriched_transactions"] + "/trx_classification"
    )

    trx_numbers_per_classification_per_week = (
        trx_classification.withColumn(
            "week_of_year", weekofyear(col("transaction_date"))
        )
        .groupBy("transaction_classification", "week_of_year")
        .count()
    )

    trx_numbers_per_classification_per_week.write.format("delta").mode(
        "overwrite"
    ).save(
        config["output"]["enriched_transactions"]
        + "/trx_numbers_per_classification_week"
    )
    ...
    trx_classification.withColumn("rn", row_number().over(window_spec)).filter(
        col("rn") == 1
    ).drop("rn").show()
